 ...
 *
 * <p>P1 起 SQLite（{@code data/farm.db}）是<b>唯一正式存档</b>，JSON 旧存档停止写入。
 * 本类把 E 的存档聚合 {@link GameState} 与 7 张表（player/player_seed/unlocked、
 * farm、soil、crop、decoration、world_state、active_event）通过 DAO 双向映射：
 * <ul>
 *   <li><b>写入</b>：事务内「先清后写、整体替换」，失败则回滚，避免半份存档；</li>
 *   <li><b>读取</b>：由 soil 与 crop 还原每块地块快照，恢复退出瞬间状态，不做任何在线成长
 *       （离线模拟在 P2）；</li>
 *   <li><b>一次性迁移</b>：首次运行若检测到旧 P0 {@code data/save.json} 且库内无玩家数据，
 *       读 JSON → 写 SQLite → 校验 → 写迁移标记，之后只用 SQLite。
 *       <b>禁止 JSON + SQLite 双写</b>（验收规范 §七十四）。</li>
 * </ul>
 *
 * <p>业务层只依赖 {@link SaveService} 接口，替换实现不影响调用方（设计 §3.2）。
 */
public class SqliteSaveService implements SaveService {
    ...
    void writeState(Connection connection, GameState state) throws SQLException {
        ...
        // （D 模块 P1 天气系统装配点经 beforeSaveHook 写入，验收 §七十三）。
        // last_real_time/random_seed 供 P2 离线模拟，P1 先写 null（列结构 §七十三 已就位）
        ...
        // active_event：当前生效事件（验收 §九十一）。无事件（event_type 为 null）时不写行，
        // 保持「0 行或 1 行」约定，读回时按无事件处理。
        if (state.getCurrentEventType() != null) {
            activeEventDao.insert(new ActiveEventDao.ActiveEventRow(
                    state.getCurrentEventType(),
                    state.getEventStartWorldTime(),
                    state.getEventEndWorldTime(),
                    state.getEventTargetCropType(),
                    state.getEventPayload()));
        }
    }

    /**
     * 全量读取：由 soil（含 crop）还原地块快照。
     *
     * @return 存档状态；库内无玩家记录时返回 {@code null}
     */
    GameState readState(Connection connection) throws SQLException {
        ...
        ActiveEventDao.ActiveEventRow activeEvent = new ActiveEventDao(connection).find();
        if (activeEvent != null) {
            state.setCurrentEventType(activeEvent.eventType());
            state.setEventStartWorldTime(activeEvent.startWorldTime());
            state.setEventEndWorldTime(activeEvent.endWorldTime());
            state.setEventTargetCropType(activeEvent.targetCropType());
            state.setEventPayload(activeEvent.payload());
        }
    ...
    /** 地图尺寸与当前版本不一致时告警（不阻断，用于提示玩家旧/新版本存档）。 */
    ...
    /** 土壤 id 口径与 A 模块 {@code BasicSoil} 一致：行 × 地图列数 + 列。 */
    ...
    /**
     * 迁移流程：库内已有玩家数据 → 只写标记（不迁移）；
     * <ul>
     *   <li>库内已有玩家数据 → 只写标记（不迁移）；</li>
     *   <li>无旧 JSON（路径为空）→ 写 {@code none} 标记，避免每次启动重复探测；</li>
     *   <li>旧 JSON 坏 → <b>不写标记</b>，给玩家修复数据的机会；</li>
     *   <li>迁移成功 → 写迁移时间标记（验收规范 §七十四「迁移完成标记」）。</li>
     * </ul>
     */
        ...
        try (Connection connection = databaseService.openConnection()) {
            ...
            try {
                ...
                try {
                    ...
                    System.err.println("[SqliteSaveService] 旧 JSON 存档无法迁移（保留数据修复机会）: "
                            + corrupt.getMessage());
                    connection.rollback();
                    return;
                }
                if (legacy != null && legacy.getPlayer() != null) {
                    ...
                    System.err.println("[SqliteSaveService] 已从旧 JSON 存档迁移到数据库: " + jsonSaveFile);
                } else {
