...
### 功能列表
  ...
  功能              描述                           依赖系统                   验收标准
  ----------------- ------------------------------ -------------------------- ------------------------------
  土地经营          12×12地图，FarmPlot支持开垦   Soil、EconomyService       EMPTY土地消耗5金币变为TILLED
...
1.  责任矩阵认领与工时：
    -   责任人、认领模块与工时数字由团队后续填入。
2.  Service命名历史差异：
    -   P0使用 BasicHarvestService；
    -   P2后由 HarvestService替代。
    -   需确认答辩文档是否保留该演进说明。
...
每次开启新任务，必须在新建对话的开头粘贴以下模板：

```text
【任务】
（填写本次任务目标，并指明所属阶段 P0～P4）

【背景资料】
请先按以下顺序阅读工作区内文档：
1. FSF游戏规则设计文档.md —— 唯一游戏规则事实源
2. FSF_P0-P4功能实现与验收规范.md —— 阶段实现与验收基准
3. 脚手架.md —— 工程结构、技术栈、分层规范
4. README.md —— 项目简介与特色系统概览
5. FSF项目需求分析与开发计划书_修订版.md —— 需求分析与开发计划

【优先级规则】
- 多个文档对同一内容说法不一致时，一律以《FSF游戏规则设计文档.md》为准
- 文档未提及的内容不得自行补充设计，一律标“待确认”
```
